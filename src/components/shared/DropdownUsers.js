...
const DropdownUsers = ({user}) => {
  const [dropdownOpen, setDropdownOpen] = useState(false);
  const router = useRouter()
  const dispatch =useAppDispatch()
  const {token,isSucces,isError,message} = useAppSelector(state=> state.userReducer)
  const logouts= async()=>{
  ...
  return (
    <ClickOutside onClick={() => setDropdownOpen(false)} className="relative">
      ...
      {dropdownOpen && (
        <div
          className={`absolute right-0 mt-3 flex w-62.5 flex-col rounded-sm border border-stroke shadow-default dark:border-strokedark dark:bg-boxdark z-10 bg-white`}
        >
