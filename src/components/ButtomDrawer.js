...
const Drawer = ({ open, setOpen, side = "bottom", sortOptions, setSelected, selected }) => {
    return (
        <div
            id={`dialog-${side}`}
                ...
                <div className="absolute inset-0 overflow-hidden">
                    ...
                    >
                        ...
                        >
                            <div
                                className={clsx(
                                    "flex flex-col h-full overflow-y-hidden bg-white  shadow-xl rounded-lg py-4 "
                                )}
                            > 
                                ...
                                {sortOptions.map((item, i) => (
                                    <div key={i} className={`py-2 px-4 cursor-pointer ${selected === item ? "text-primary " : ""}`} style={selected === item ? { background: 'rgba(0, 172, 187, .08)' } : {}} onClick={() => setSelected(item)}>{item}</div>
                                ))}
                            </div>
                        </div>
                    </div>
